...
function setGame(name) {
  var game = {type: 1, name: name, url: "http://www.twitch.tv/firec123123"};
...
client.Dispatcher.on("GATEWAY_READY", e => {
  ...
  bf.config({memorySize: 30000, bits: 8});
});
...
try {
	var code = []
	var bfded = bf.compile(e.message.content.substring(16, 9999999999999));
	bfded.run(bfded, function (num, chor) {
  code.push(chor);
});
     var finishdcode = code.join('');
	if (!finishdcode == "") {
	e.message.channel.sendMessage(finishdcode);
	}
	if (finishdcode == "") {
	e.message.channel.sendMessage("My BF has an error o no :broken_heart:");
}	
}
catch (err) {
e.message.channel.sendMessage("My BF Crashed With Error : " + err.toString() + " :broken_heart: ");	
}

}
  if (e.message.content =="*changelog")
    e.message.channel.sendMessage("`10/12/2016`\nadded a *quote command\n`10/5/2016`\nnow anyone that says (@)everyone or (@)here will be logged\nadded a *givemearole command'");
